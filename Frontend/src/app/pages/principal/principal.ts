...
interface Stats { vehiculosActivos: number; rutasCompletadas: number; eficiencia: number; kmRecorridos: number; }
interface FormRuta { nombre: string; zona: string; horario: string; }
interface FormVehiculo { placa: string; modelo: string; marca: string; activo: boolean; }
interface FormDireccion { rutaId: number; direccion: string; orden: number; }
...
export class PrincipalComponent implements OnInit {
  ...
  cargarRutas() {
    this.rutasService.getRutas().subscribe({
      next: (resp: any) => {
        console.log('Rutas recibidas:', resp); // Para depuración
  ...
  actualizarStats() {
    ...
    const rutasCompletadas = this.rutas().filter(r => r.estado === 'completada').length;
  ...
  abrirModalRegistrarRuta() { this.formRuta.set({ nombre: '', zona: '', horario: '' }); this.showRegistrarRutaModal.set(true); }
  cerrarModalRegistrarRuta() { this.showRegistrarRutaModal.set(false); }
  trazarRuta() { this.showRouteTools.set(true); this.showRegistrarRutaModal.set(false); }

  abrirModalRegistrarVehiculo() { this.formVehiculo.set({ placa: '', modelo: '', marca: '', activo: true }); this.showRegistrarVehiculoModal.set(true); }
  cerrarModalRegistrarVehiculo() { this.showRegistrarVehiculoModal.set(false); }

  abrirModalIngresarDireccion() { this.formDireccion.set({ rutaId: this.rutas()[0]?.id || 0, direccion: '', orden: 1 }); this.showIngresarDireccionModal.set(true); }
  ...
  guardarRuta() {
    ...
    this.rutasService.guardarRuta({ nombre: form.nombre, zona: form.zona, horario: form.horario, estado: 'programada' }).subscribe({
      next: () => { this.cargarRutas(); this.cerrarModalRegistrarRuta(); },
  ...
  guardarDireccion() {
    ...
    if (!form.direccion || form.rutaId === 0) { alert('Completa todos los campos'); return; }
